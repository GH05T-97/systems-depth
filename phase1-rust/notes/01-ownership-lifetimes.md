...
Using the `&` operator creates an immutable reference, meaning the variable that's doing the borrowing cannot mutate the original value. So how can we mutate the value by using the borrower?
